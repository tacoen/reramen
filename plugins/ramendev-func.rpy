init -10 python:
    ...
    def collector(what,obj_type_like):
        ...
        try: ramen.objects[what]
        except:  ramen.objects[what]=[]
    ...
    def rtextformat(var,step=-1,pre='\n',res=''):
        ...
        if isinstance(var,(type({}),type(tes.__dict__))):
            for v in var:
                res += pre + v +' {space=10}={space=10} '+ rtextformat(var[v],step,'\n')
            ...
            for v in var:
                ...
                r += nn +"{color=#999}" + str(var.index(v)) + '{/color} = {color=#9CF}' + ve +'{/color}\n'
                nn = ""
